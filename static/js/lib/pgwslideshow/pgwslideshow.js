    ...
    $.fn.pgwSlideshow = function(options) {
        ...
        var setup = function() {
            ...
            pgwSlideshow.plugin.find('.ps-list > ul > li').each(function() {
                ...
                element.num = elementId;
                pgwSlideshow.data.push(element);

                $(this).addClass('elt_' + element.num);
                ...
                if (element.image) {
                    currentElement.html('<img src="' + element.image + '" alt="' + (element.title ? element.title : '') + '" id="' + (element.id ? element.id : '') + '">');
                } else if (element.thumbnail) {
                    currentElement.html('<img src="' + element.thumbnail + '" alt="' + (element.title ? element.title : '') + '" id="' + (element.id ? element.id : '') + '">');
                }

                if (element.link) {
                    currentElement.html('<a href="' + element.link + '"' + (element.linkTarget ? ' target="' + element.linkTarget + '"' : '') + ' class="icon-video">' + currentElement.html() + '</a>');
                }
        ...
        var getElement = function(obj) {
            ...
            // Get operate ,newfunction add by jzm
            var elementOperate = obj.find('img').attr('data-operate');
            if ((typeof elementOperate != 'undefined') && (elementOperate != '')) {
                element.operate = elementOperate;
            }

            // Get id
            var elementId = obj.find('img').attr('id');
        ...
        var finishElement = function(element) {
            ...
            //new add by jzm
            if (element.operate) {
                elementText += '<span class="btn btn-success float-right js-del margin-left-10">删除</span>';
                // if (element.operate ==1) {
                //     elementText += '<span class="btn btn-success float-right js-down margin-left-10">下载</span><span class="btn btn-success float-right js-submit margin-left-10">提交样板间</span>';
                // } else if (element.operate ==3){
                //     elementText += '<span class="btn btn-success float-right js-down margin-left-10">下载</span>';
                // }
            ...
            pgwSlideshow.plugin.find('.ps-list > ul > li.elt_' + element.num + ' .ps-item').addClass('ps-selected');
            ...
            if (typeof pgwSlideshow.config.afterSlide == 'function') {
                pgwSlideshow.config.afterSlide(element.num);
            }

            // Set the container height
            var maxHeight = pgwSlideshow.plugin.find('.ps-current .elt_' + element.num + ' img').height();
        ...
        var fadeElement = function(element) {
            ...
            elementContainer.find('li').not('.elt_' + pgwSlideshow.currentSlide).not('.elt_' + element.num).each(function(){
            ...
            // Update current id
            pgwSlideshow.currentSlide = element.num;

            // Next element
            var nextElement = elementContainer.find('.elt_' + element.num);
